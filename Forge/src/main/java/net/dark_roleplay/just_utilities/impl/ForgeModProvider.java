...
public class ForgeModProvider implements IModInterface {
	...
	public Path[] getRootPathsForMod(String modid) {
		...
		if(file == null)
			return new Path[0];
		return new Path[]{file.getFile().getFilePath()};
	}
	...
	public void setLoadingContext(String modid){
		ModContainer container = ModList.get() != null ?
				...
				new EarlyModContainer(LoadingModList.get().getModFileById(modid).getMods().stream().filter(info -> info.getModId().equals(modid)).findFirst().orElseThrow());
